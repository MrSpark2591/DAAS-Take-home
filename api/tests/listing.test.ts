...
import { type Fixtures, givenPurchaseOrder, givenStock, prisma, seedFixtures } from './helpers.js';
...
describe('paging through purchase orders', () => {
  ...
  it('rejects a page size beyond the cap', async () => {
    ...
    await expect(listPurchaseOrderIds({}, { first: 0 })).rejects.toThrow(/positive whole number/i);
  });
...
describe('filtering purchase orders', () => {
  it('combines filters with AND rather than widening', async () => {
  ...
  it('matches a PO number substring, case-insensitively', async () => {
  ...
  it('excludes soft-deleted purchase orders', async () => {
...
describe('filtering stock on hand', () => {
  ...
  it('hides zero rows when asked', async () => {
  ...
  it('searches SKU and product name together', async () => {
  ...
  it('orders by SKU and pages without overlap', async () => {
